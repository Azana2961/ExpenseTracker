...
def main():
    path = r'd:\HostelExpenseApp\frontend\lib\features\dashboard\presentation\dashboard_screen.dart'
    ...
    wallet_chunk = """              // 1 ── Wallet Row (Teal card + Loans/Borrows) ──────────────
              _WalletRow(
    ...
    new_wallet = """              // 1 ── Wallet Row (Teal card + Loans/Borrows) ──────────────
              _WalletRow(
                selectedDaySpent: selectedDaySpent,
                budgetPercentage: budgetPercentage,
                hasActiveLoan: hasActiveLoan,
              ...
              const SizedBox(height: 20),"""
    content = content.replace(wallet_chunk, new_wallet)

    # 6. Delete _buildMonthlyBudgetSection and _buildInfoBox using regex
    content = re.sub(r'  // ─── Monthly Budget Section.*?  // ─── Loan / Borrow bottom sheet', '  // ─── Loan / Borrow bottom sheet', content, flags=re.DOTALL)
    ...
    old_wallet_row_class = re.search(r'class _WalletRow extends StatelessWidget \{.*?\n\}\n', content, flags=re.DOTALL).group(0)
    new_wallet_row_class = """class _WalletRow extends StatelessWidget {
...
"""
    content = content.replace(old_wallet_row_class, new_wallet_row_class)
    ...
    content = content.replace(
        "onLongPress: isToday ? onEdit : null,",
    ...
    content = content.replace(
        "onTap: isSpecial && !isCleared && isToday ? onRecordPayment : null,",
